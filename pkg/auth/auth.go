...
func UpdateUser(c *gin.Context) {
	_ = db.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := utils.AddChangelogsForUser(tx, username, &olduser, &updatedUser); err != nil {
			er := models.LicenseError{
				Status:    http.StatusInternalServerError,
				Message:   "Failed to update user",
				Error:     err.Error(),
...
func DeleteUser(c *gin.Context) {
	_ = db.DB.Transaction(func(tx *gorm.DB) error {
		...
		updatedUser := olduser
		fmt.Println(olduser.Active)
		newActive := false
		updatedUser.Active = &newActive
		fmt.Println(updatedUser.Active)
