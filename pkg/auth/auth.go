...
//	@Accept			json
//	@Produce		json
//	@Param			user	body		models.OidcUserCreate	true	"User to create"
//	@Success		201		{object}	models.UserResponse
//	@Failure		400		{object}	models.LicenseError	"Invalid json body"
//	@Failure		409		{object}	models.LicenseError	"User already exists"
...
func CreateOidcUser(c *gin.Context) {
	...
	var tokenBody models.OidcUserCreate
	...
	keyError := true
	if kid, err := utils.GetKid(tokenBody.Token); err == nil {
	...
	if _, err = jws.Verify([]byte(tokenBody.Token), keyOptions); err != nil {
		er := models.LicenseError{
			Status:    http.StatusUnauthorized,
			Message:   "Please check your credentials and try again",
			Error:     "token verification failed",
	...
	parsedToken, err := jwt.Parse([]byte(tokenBody.Token), jwt.WithValidate(true), jwt.WithVerify(false))
	if err != nil {
