...
// OidcClient struct for Api Layer(POST).
type CreateOidcClientDTO struct {
	Id          uuid.UUID `json:"-"`
	ClientId    string    `validate:"required" example:"qwerty"`
	Name        string    `json:"name" example:"FOSSology Integration Client"`
	Description string    `json:"description" example:"Client used by CI pipeline to authenticate with LicenseDb"`
	UserId      uuid.UUID `json:"-"`
	User        User      `json:"-"`
	AddDate     time.Time `json:"-"`
}

// OidcClient struct for Api Layer(DELETE).
type DeleteOidcClientDTO struct {
	Id          uuid.UUID `json:"-"`
	ClientId    string    `validate:"required" example:"qwerty"`
	Name        string    `json:"-"`
