...
func getSimilarLicense(c *gin.Context) {
	...
	if thresholdStr != "" {
		if parsed, err := strconv.ParseFloat(thresholdStr, 64); err == nil {
			...
			log.Printf("Invalid SIMILARITY_THRESHOLD '%s', using default 0.7", thresholdStr)
		}
	} else {
		log.Println("SIMILARITY_THRESHOLD not set, using default 0.8")
	}
	...
	// Set threshold
	setQuery := fmt.Sprintf("SET pg_trgm.similarity_threshold = %f", similarityThreshold)
	if err := db.DB.Exec(setQuery).Error; err != nil {
		log.Println("Failed to set threshold:", err)
	}
